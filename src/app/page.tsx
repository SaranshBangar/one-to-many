...
const TESTIMONIALS = [
  {
    quote: "I was blown away by how well it captured my voice. It’s like having a personal content team that just gets me",
    name: "Saransh Bangar",
    role: "Doodledangeler @ Bengaluru",
  },
];
    ...
    <div className="flex flex-col">
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col items-center px-4 py-20 text-center md:py-28">
        <h1 className="display mx-auto mt-6 max-w-3xl text-5xl md:text-[64px]">
          Turn your podcast into <span className="text-accent">10 posts</span> in 2 minutes
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-lg text-muted md:text-[19px]">
          Built for founders. Preserves your voice. No generic fluff, upload once, copy-paste everywhere.
        </p>
        ...
        <p className="mt-3 font-mono text-xs text-muted-2">No card required · cancel anytime</p>
      </section>

      {/* Social proof */}
      <section className="border-y border-border bg-surface/40">
        <div className="mx-auto max-w-6xl px-4 py-16">
          <p className="text-center font-mono text-[13px] text-muted-2">Used by 1 founder(s) shipping content</p>
          <div className="mt-8 flex flex-col items-center justify-center">
            {TESTIMONIALS.map((t) => (
              <figure key={t.name} className="flex flex-col justify-between gap-5 rounded-[var(--radius-card)] border border-border bg-surface p-6">
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-20">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {PLATFORM_KEYS.map((p) => (
            <div key={p} className="flex flex-col gap-4 rounded-[var(--radius-card)] border border-border bg-surface p-6">
              <PlatformGlyph id={p} size={42} />
              <div className="flex flex-col gap-1.5">
                <h3 className="font-semibold">{PLATFORMS[p].short}</h3>
                <p className="text-[13.5px] leading-relaxed text-muted">{PLATFORMS[p].blurb}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-4 py-10 text-sm text-muted sm:flex-row">
          <Logo />
          <p className="font-mono text-xs text-muted-2">Made by an indie founder.</p>
        </div>
      </footer>
    </div>
  );
